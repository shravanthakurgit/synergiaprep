...
const handleStartTest = (test : object) => {
  ...
  const uniqueSubjects = useMemo(() => {
    if (!pyqTests || !Array.isArray(pyqTests)) return [];
    const subjects = pyqTests.flatMap((test) =>
  ...
  const availableExams = useMemo(() => {
    if (!pyqTests || !Array.isArray(pyqTests)) return [];
    return [...new Set(pyqTests.map((test) => test.exam))];
  }, [pyqTests]);

  // Filter tests based on selected filters
  const filteredExams = useMemo(() => {
    if (!pyqTests || !Array.isArray(pyqTests)) return [];
    return pyqTests.filter(
      (test) =>
  ...
  }, [pyqTests, selectedExam, selectedSubject, searchQuery]);
        ...
        
{
  ...
  const renderTestRow = (test: pyqTest) => {
    const totalChapters = test.subjects.flatMap((s) => s.chapters).length;

    return (
      <div key={test.id}>
        <Card className="hover:shadow-lg transition-all duration-300 border hover:border-emerald-200 bg-white mb-3">
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="my-[-2rem] bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-100 py-12 text-center mb-8 rounded-lg shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {selectedExam && (
              <Badge
                variant="secondary"
                className="flex gap-1 items-center bg-emerald-100 text-emerald-700"
              >
                Exam: {selectedExam}
                <Button
                  variant="ghost"
