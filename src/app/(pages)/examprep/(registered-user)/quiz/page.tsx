...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchQuizTests = async () => {
      setLoading(true);
      try {
        const response = await fetch("/api/v1/exams?type=quiz");
        const data = await response.json();
        setQuizTests(data.data as quizTest[]);
      } catch (e) {
  ...
  const handleStartTest = (testId: string) => {
    if (isLoggedIn) {
      ...
      router.push(`/exam?examId=${testId}&type=quiz`);
    } else {
      // User is not logged in, redirect to login with callback
      const callbackUrl = `/exam?examId=${testId}&type=quiz`;
  ...
  const uniqueSubjects = useMemo(() => {
    if (!quizTests || !Array.isArray(quizTests)) return [];
    const subjects = quizTests.flatMap((test) =>
  ...
  const availableExams = useMemo(() => {
    if (!quizTests || !Array.isArray(quizTests)) return [];
    return [...new Set(quizTests.map((test) => test.exam))];
  }, [quizTests]);

  // Filter tests based on selected filters
  const filteredExams = useMemo(() => {
    if (!quizTests || !Array.isArray(quizTests)) return [];
    return quizTests.filter(
      (test) =>
  ...
  }, [quizTests, selectedExam, selectedSubject, searchQuery]);
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-4">
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 py-16 text-center mb-8 rounded-lg shadow-sm border">
  ...
  const renderTestCard = (test: quizTest) => {
          ...
          <CardContent className="flex-grow">
            ...
            {!isLoggedIn && (
              <div className="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 p-2 rounded-lg mb-2">
                ...
                <span>Login required to attempt this quiz</span>
              </div>
            )}
          </CardContent>
...
/>
  ...
  const renderTestRow = (test: quizTest) => {
    const totalChapters = test.subjects.flatMap((s) => s.chapters).length;
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {selectedSubject && (
              <Badge
                variant="secondary"
                className="flex gap-1 items-center bg-indigo-100 text-indigo-700"
              >
