...
//     try {
//       // Remove existing image first if exists
//       if (image) {
//         await removeImage();
//       }
...
//     // Phone validation - must be empty or exactly 10 digits
...
//       // Handle phone number - send null if empty, number if valid
//       if (phone.trim() === "") {
...
//       // Call your API route to update the user
//       const response = await fetch('/api/users', {
//         method: 'PATCH',
//         headers: {
//           'Content-Type': 'application/json',
  ...
  const uploadFile = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        const smallerPayload = await getResizedImage(
          fileData as ArrayBuffer
        );

        await uploadImg(presignedURL.toString(), smallerPayload, file);
